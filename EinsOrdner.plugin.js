 ...
 * @author EinsMarvs
 * @version 2.0
 * @description Lässt deine Server in Benutzerdefinierte Ordner unterteilen | Made by EinsMarvs
 * @updated on 17.08.2021, 00:10
 */
 ...
 module.exports = (_ => {
	const config = {
		...
		"changeLog": {
			"fixed": {
				"Neues: | Update 2.0 veröffentlicht": "Update 2.0: Verbesserung der Config & Crash fixes durchgeführt"
			},
		}
	};
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		getSettingsPanel () {
			...
			template.innerHTML = `<div style="color: var(--header-primary); font-size: 16px; font-weight: 300; white-space: pre; line-height: 22px;">Das für ${config.info.name} benötigte Library Plugin fehlt.\nBitte klicken Sie auf <a style="font-weight: 500;">Jetzt herunterladen</a>, um es zu installieren.</div>`;
										...
										if (openInput.props.value && closeInput.props.value) {
											this.checkImage(openInput.props.value, openIcon => {
												this.checkImage(closeInput.props.value, closedIcon => {
													...
													BDFDB.NotificationUtils.toast("Benutzerdefiniertes Symbol wurde zur Auswahl hinzugefügt", {type: "success"});
												});
											})
										}
										else BDFDB.NotificationUtils.toast("Hinzufügen eines Bildes für das offene und geschlossene Symbol", {type: "danger"});
									}
								})
							]
						})
		...
		return class ServerFolders extends Plugin {
			onLoad () {
				...
				this.defaults = {
					general: {
						closeOtherFolders:	{value: false, 	description: "Schließt andere Ordner beim Öffnen eines Ordners"},
						closeTheFolder:		{value: false, 	description: "Schließt den Ordner bei Auswahl eines Servers"},
						closeAllFolders:	{value: false, 	description: "Schließt alle Ordner bei Auswahl eines Servers"},
						forceOpenFolder:	{value: false, 	description: "Erzwingt das Öffnen eines Ordners beim Wechsel zu einem Server dieses Ordners"},
						showCountBadge:		{value: true, 	description: "Zeigt das Abzeichen für die Anzahl der Server in einem Ordner an"},
						extraColumn:		{value: true, 	description: "Verschiebt die Server aus geöffneten Ordnern in eine extra Spalte"},
						addSeparators:		{value: true, 	description: "Fügt Trennzeichen zwischen Servern verschiedener Ordner in extra Spalte ein"}
					}
				};
				...
				return settingsPanel = BDFDB.PluginUtils.createSettingsPanel(this, {
					...
					children: _ => {
						...
						settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
							...
							label: "Alle Ordner zurücksetzen",
							onClick: _ => BDFDB.ModalUtils.confirm(this, "Sind Sie sicher, dass Sie alle Ordner zurücksetzen wollen?", _ => BDFDB.DataUtils.remove(this, "folders")),
							children: BDFDB.LanguageUtils.LanguageStrings.RESET
						}));
						settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
							type: "Button",
							color: BDFDB.LibraryComponents.Button.Colors.RED,
							label: "Alle benutzerdefinierte Symbole entfernen",
							onClick: _ => BDFDB.ModalUtils.confirm(this, "Sind Sie sicher, dass Sie alle benutzerdefinierten Symbole entfernen möchten?", _ => BDFDB.DataUtils.remove(this, "customicons")),
				...
				if (e.instance.props.target && e.instance.props.folderId) {
					...
					e.returnvalue.props.children.push(BDFDB.ContextMenuUtils.createItem(BDFDB.LibraryComponents.MenuItems.MenuGroup, {
						children: BDFDB.ContextMenuUtils.createItem(BDFDB.LibraryComponents.MenuItems.MenuItem, {
							...
							color: BDFDB.LibraryComponents.MenuItems.Colors.DANGER,
							action: event => {
								BDFDB.ModalUtils.confirm(this, `Sind Sie sicher, dass Sie den Ordner entfernen wollen${folder.folderName ? ` '${folder.folderName}'` : ""}?`, _ => {
